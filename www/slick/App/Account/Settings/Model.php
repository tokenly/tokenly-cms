...
{
	public function getSettingsForm($user, $app, $adminView = false)
	{
	...
	public function updateSettings($user, $data, $app, $isAPI = false, $adminView = false)
	{
		...
		if(isset($data['field-'.PRIMARY_TOKEN_FIELD]) AND trim($data['field-'.PRIMARY_TOKEN_FIELD]) != ''){
			...
			if($getField){
				...
				if($update){
					$addressModel = new Slick_App_Dashboard_LTBcoin_Address_Model;
