...
<div class="thread-op">
	...
	<div class="op-content">
		<div class="post-content" data-user-slug="<?= $topic['author']['slug'] ?>" data-message="<?= base64_encode($topic['content']) ?>">
			<?php
			if($topic['trollPost'] == 1){
				echo '<p class="text-error">This post is hidden from public view and must be approved by an admin or moderator.</p>';
			}
			?>
	...
	if($user){
		...
		$topic['author']['meta'] = $meta->userMeta($topic['userId']);
		if($perms['canApproveUsers'] AND isset($topic['author']['meta']['needs_forum_approval']) AND intval($topic['author']['meta']['needs_forum_approval']) == 1){
			echo '<a href="'.SITE_URL.'/'.$app['url'].'/approve-user/'.$topic['userId'].'" target="_blank">Approve User</a>';
		}
	...
	foreach($replies as $k => $reply){
		...
		<?php
			...
			if($user AND $reply['buried'] != 1){
				...
				$reply['author']['meta'] = $meta->userMeta($reply['userId']);
				if($perms['canApproveUsers'] AND isset($reply['author']['meta']['needs_forum_approval']) AND intval($reply['author']['meta']['needs_forum_approval']) == 1){
					echo '<a href="'.SITE_URL.'/'.$app['url'].'/approve-user/'.$reply['userId'].'" target="_blank">Approve User</a>';
				}
	...
	if($user){
		...
		elseif(isset($form)){
			if(isset($_POST['content'])){
				$form->field('content')->setValue($_POST['content']);
			}
			echo $form->display();
