...
<h2>The Newsroom</h2>
<?= $this->displayBlock('dashboard-newsroom') ?>
<?php
	echo '<div class="newsroom-filter"><label>Filter All Posts:</label><select class="status_filter">
...
foreach($blogs as $blog){
	if(isset($blog_rooms[$blog['blogId']]) AND count($blog_rooms[$blog['blogId']]) > 0){
		...
		echo '<table class="admin-table data-table '.$blog['slug'].'_posts submissions-table">
				...
				<tbody>';
		foreach($blog_rooms[$blog['blogId']] as $post){
